...
def summary(
    ...
    binarization_base_class: Optional[Type] = None,
    **kwargs: Any,
) -> ModelStatistics:
    """
    ...
    summary_list = forward_pass(
        model, x, batch_dim, cache_forward_pass, device, quantization_base_class=quantization_base_class, binarization_base_class = binarization_base_class, **kwargs
    )
...
def forward_pass(
    ...
    **kwargs: Any,
    ...
    print(quantization_base_class)
    print(binarization_base_class)
    ...
    apply_hooks(named_module, model, batch_dim, summary_list, {}, hooks, quantization_base_class=quantization_base_class, binarization_base_class = binarization_base_class)
...
def apply_hooks(
    ...
    def pre_hook(module: nn.Module, inputs: Any) -> None:
        ...
        info.determine_if_quantized(quantization_base_class = quantization_base_class, binarization_base_class = binarization_base_class)
    ...
    for child in module._modules.items():  # pylint: disable=protected-access
        apply_hooks(
            child, orig_model, batch_dim, summary_list, idx, hooks, curr_depth + 1, info, quantization_base_class=quantization_base_class, binarization_base_class = binarization_base_class
        )
