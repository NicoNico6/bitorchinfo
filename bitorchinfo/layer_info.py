...
class LayerInfo:
    ...
    def determine_if_quantized(self, quantization_base_class: Optional[Type] = None, binarization_base_class: Optional[Type] = None) -> None:
        ...
        """
        if quantization_base_class or binarization_base_class:
            attribute_types = [type(getattr(self.module, attribute)) for attribute in dir(self.module)]
            if quantization_base_class is not None and isinstance(self.module, quantization_base_class):
              self.quantized = True #any([issubclass(attribute_type, quantization_base_class) for attribute_type in attribute_types])
            else:
              self.quantized = False
              
            if binarization_base_class is not None and isinstance(self.module, binarization_base_class):
              self.binarized = True #any([issubclass(attribute_type, binarization_base_class) for attribute_type in attribute_types])
            else:
              self.binarized = False
